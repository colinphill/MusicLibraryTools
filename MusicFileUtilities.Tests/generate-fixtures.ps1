...
.SYNOPSIS
  ...
  fixture uses a committed tone stream generated by the official BSD-licensed SDK because
  ffmpeg provides a decoder but no APE encoder.
...
# --- Monkey's Audio reference fixture and baseline APEv2 tag -------------------------------
...
$apeReference = Join-Path $PSScriptRoot 'ReferenceFixtures\MonkeysAudio\sample.ape'
if (-not (Test-Path $apeReference)) {
    throw "Required official Monkey's Audio fixture is missing: $apeReference."
}
Copy-Item -LiteralPath $apeReference -Destination $apePath -Force
[byte[]]$apeTag = New-ApeV2Tag
$apeStream = [System.IO.File]::Open(
    $apePath,
    [System.IO.FileMode]::Append,
    [System.IO.FileAccess]::Write,
    [System.IO.FileShare]::None)
try {
    $apeStream.Write($apeTag, 0, $apeTag.Length)
}
finally {
    $apeStream.Dispose()
}
...
if (-not (Test-Path $dsfPath) -or
    (Get-Item -LiteralPath $dsfPath).Length -lt 8192) {
    $channels      = 2
    $blockPerChan  = 4096
    ...
    # Cast the repeated values to byte[] explicitly so BinaryWriter selects
    # Write(byte[]) instead of treating the untyped PowerShell array as one
    # scalar value.
    $bw.Write([byte[]](,[byte]0x69 * $dataPayload))
