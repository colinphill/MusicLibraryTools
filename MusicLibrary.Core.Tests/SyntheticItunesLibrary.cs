...
/// <summary>Small structurally valid ITL used to exercise Core's transactional integration.</summary>
internal static class SyntheticItunesLibrary
{
    public static byte[] CreateFile(string mediaFolder)
    {
        byte[] body = CreateBody(mediaFolder);
        return ItlWriter.Build(CreateEnvelope(body), body);
    }

    private static ItlEnvelope CreateEnvelope(byte[] body)
    {
    ...
    private static byte[] CreateBody(string mediaFolder)
    {
        byte[] mfdh = Chunk("mfdh", [], headerLength: 144);
        byte[] folder = Chunk("mhoh",
            Encoding.Unicode.GetBytes(Path.GetFullPath(mediaFolder) + "\0"),
            headerLength: 24);
        BinaryPrimitives.WriteInt32LittleEndian(
            folder.AsSpan(12), (int)ItlDataType.LibraryFolderPath);
        byte[] mhgh = Chunk("mhgh", folder, headerLength: 128, countAt8: 1);
        ...
        BinaryPrimitives.WriteUInt32LittleEndian(
            trackHeader.AsSpan(ItlDocument.TrackSecondaryIdOffset), 2);
        BinaryPrimitives.WriteUInt64LittleEndian(
            trackHeader.AsSpan(128), 0x1111111111111111);
        byte[] cloudHeader = (byte[])trackHeader.Clone();

        ItlField name = ItlField.CreateString(
            (int)ItlDataType.PlaylistName, "####!####");
        ...
        BinaryPrimitives.WriteUInt64LittleEndian(
            entry.AsSpan(68), 0x4444444444444444);
        ...
        BinaryPrimitives.WriteUInt64LittleEndian(
            playlistHeader.AsSpan(ItlDocument.PlaylistPersistentIdOffset),
            0x5555555555555555);
        BinaryPrimitives.WriteUInt32LittleEndian(
            playlistHeader.AsSpan(ItlDocument.PlaylistRecordIdOffset), 6);
    ...
    {
        byte[] list = new byte[12 + records.Sum(record => record.Length)];
    ...
    private static byte[] RecordHeader(string signature, int length, uint id)
    {
        ...
        if (signature != "miph")
            BinaryPrimitives.WriteUInt32LittleEndian(header.AsSpan(16), id);
        return header;
    }
        ...
        int? countAt8 = null)
    {
        ...
        BinaryPrimitives.WriteInt32LittleEndian(
            bytes.AsSpan(8), countAt8 ?? bytes.Length);
