...
public partial class LibraryView : UserControl
{
    ...
    private readonly StudioGridStateService _gridState;
    private readonly IPlatformService _platform;
    private readonly List<StudioGridColumnDefinition> _columns = [];
    ...
    public LibraryView()
    {
        ...
        _gridState = App.GetService<StudioGridStateService>();
    ...
    private void ApplySnapshot(StudioGridSnapshot? snapshot)
    {
        if (snapshot is null)
            return;
        foreach (LibraryColumnState saved in snapshot.Columns.OrderBy(item => item.DisplayIndex))
        {
            ...
            if (index < 0)
                continue;
            StudioGridColumnDefinition current = _columns[index];
    ...
    private void BuildColumnOptions()
    {
        ...
        foreach (StudioGridColumnDefinition definition in _columns)
        {
    ...
    private IReadOnlyList<LibraryColumnState> CaptureColumns()
    {
        ...
        foreach (StudioGridColumnDefinition definition in _columns.OrderBy(item =>
    ...
    private void PersistLayout() => _gridState.Save(new StudioGridSnapshot(CaptureColumns(), _sort));
