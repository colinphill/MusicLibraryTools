...
public sealed record StudioGridColumnDefinition(
    string Key,
    string Header,
    string? BindingPath,
    double Width,
...
public sealed class StudioDataGrid : DataGrid
{
    private readonly Dictionary<DataGridColumn, StudioGridColumnDefinition> _definitions = [];
    ...
    public StudioDataGrid()
    {
        Classes.Add("studio-grid");
    ...
    public void ConfigureColumns(IEnumerable<StudioGridColumnDefinition> definitions)
    {
        _configuring = true;
        try
        {
            ...
            _definitions.Clear();
            Columns.Clear();
            foreach (StudioGridColumnDefinition definition in definitions.Where(item => item.Visible))
            {
    ...
    public IReadOnlyList<LibraryColumnState> CaptureColumnLayout() => Columns
        .Select(column =>
        {
            StudioGridColumnDefinition definition = _definitions[column];
    ...
    public string? KeyFor(DataGridColumn column) =>
        _definitions.TryGetValue(column, out StudioGridColumnDefinition? definition) ? definition.Key : null;
