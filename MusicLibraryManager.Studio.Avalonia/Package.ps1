...
$project = Join-Path $PSScriptRoot "MusicLibraryManager.Studio.Avalonia.csproj"
if ([string]::IsNullOrWhiteSpace($OutputRoot)) {
    $OutputRoot = Join-Path $projectRoot ".artifacts/studio-avalonia"
}
...
foreach ($rid in $Rids) {
    ...
    $productName = "MusicLibraryManager.Studio.Avalonia-$Version-$rid"
    ...
    elseif ($rid.StartsWith("osx-", [StringComparison]::OrdinalIgnoreCase)) {
        $bundle = Join-Path $packageRoot "MusicLibraryManager Studio.app"
...
<plist version="1.0"><dict>
  <key>CFBundleExecutable</key><string>MusicLibraryManager.Studio.Avalonia</string>
  <key>CFBundleIdentifier</key><string>com.musiclibrarytools.studio</string>
  <key>CFBundleName</key><string>Music Library Manager Studio</string>
  <key>CFBundleDisplayName</key><string>Music Library Manager Studio</string>
...
"@
        ...
        if (Test-Path -LiteralPath $archive) { Remove-Item -LiteralPath $archive -Force }
        & tar -czf $archive -C $packageRoot "MusicLibraryManager Studio.app"
