...
internal static partial class CatalogGenerator
{
    ...
    private static readonly string ExactResourceTranslationRows =
        ...
        Transcode.Title|Audio transkodieren|Transcodificar audio|Transcoder l’audio|Transcodifica audio|Transcodificar áudio|オーディオを変換|오디오 트랜스코딩|转码音频|轉碼音訊
        Transcode.Action.Open|Transkodieren…|Transcodificar…|Transcoder…|Transcodifica…|Transcodificar…|変換…|트랜스코딩…|转码…|轉碼…
        Transcode.Action.Close|Transkodierungseditor schließen|Cerrar editor de transcodificación|Fermer l’éditeur de transcodage|Chiudi editor di transcodifica|Fechar editor de transcodificação|変換エディターを閉じる|트랜스코딩 편집기 닫기|关闭转码编辑器|關閉轉碼編輯器
        Transcode.Action.ApplyReady|Bereite Dateien anwenden|Aplicar archivos listos|Appliquer les fichiers prêts|Applica file pronti|Aplicar arquivos prontos|準備済みファイルを適用|준비된 파일 적용|应用就绪文件|套用已就緒檔案
        Transcode.Action.ReplacePending|Ausstehende Transkodierungen ersetzen|Reemplazar transcodificaciones pendientes|Remplacer les transcodages en attente|Sostituisci transcodifiche in sospeso|Substituir transcodificações pendentes|保留中の変換を置換|보류 중인 트랜스코딩 교체|替换待处理转码|取代待處理轉碼
        Transcode.Selection.None|Keine Dateien ausgewählt|No hay archivos seleccionados|Aucun fichier sélectionné|Nessun file selezionato|Nenhum arquivo selecionado|ファイルが選択されていません|선택한 파일 없음|未选择文件|未選取檔案
        Transcode.Selection.Files.One|{0:N0} Datei erfasst|{0:N0} archivo capturado|{0:N0} fichier capturé|{0:N0} file acquisito|{0:N0} arquivo capturado|{0:N0} ファイルを取得|{0:N0}개 파일 캡처됨|已捕获 {0:N0} 个文件|已擷取 {0:N0} 個檔案
        Transcode.Selection.Files.Other|{0:N0} Dateien erfasst|{0:N0} archivos capturados|{0:N0} fichiers capturés|{0:N0} file acquisiti|{0:N0} arquivos capturados|{0:N0} ファイルを取得|{0:N0}개 파일 캡처됨|已捕获 {0:N0} 个文件|已擷取 {0:N0} 個檔案
        Transcode.Preset.Title|Voreinstellung|Preajuste|Préréglage|Preimpostazione|Predefinição|プリセット|프리셋|预设|預設
        Transcode.Preset.Custom|Benutzerdefinierte Einstellungen|Configuración personalizada|Paramètres personnalisés|Impostazioni personalizzate|Configurações personalizadas|カスタム設定|사용자 지정 설정|自定义设置|自訂設定
        Transcode.Preset.Name|Name der Voreinstellung|Nombre del preajuste|Nom du préréglage|Nome preimpostazione|Nome da predefinição|プリセット名|프리셋 이름|预设名称|預設名稱
        Transcode.Preset.SaveAs|Speichern unter|Guardar como|Enregistrer sous|Salva con nome|Salvar como|名前を付けて保存|다른 이름으로 저장|另存为|另存新檔
        Transcode.Preset.Update|Aktualisieren|Actualizar|Mettre à jour|Aggiorna|Atualizar|更新|업데이트|更新|更新
        Transcode.Preset.DeleteTitle|Transkodierungsvoreinstellung löschen?|¿Eliminar el preajuste de transcodificación?|Supprimer le préréglage de transcodage ?|Eliminare la preimpostazione di transcodifica?|Excluir predefinição de transcodificação?|変換プリセットを削除しますか？|트랜스코딩 프리셋을 삭제할까요?|删除转码预设？|刪除轉碼預設？
        Transcode.Preset.DeleteMessage|Dadurch wird die ausgewählte Voreinstellung entfernt. Audiodateien werden nicht geändert.|Esto elimina el preajuste seleccionado. No se cambia ningún archivo de audio.|Cela supprime le préréglage sélectionné. Aucun fichier audio n’est modifié.|Rimuove la preimpostazione selezionata. Nessun file audio viene modificato.|Isso remove a predefinição selecionada. Nenhum arquivo de áudio é alterado.|選択した名前付きプリセットを削除します。オーディオファイルは変更されません。|선택한 이름 지정 프리셋을 제거합니다. 오디오 파일은 변경되지 않습니다.|这将删除所选命名预设，不会更改任何音频文件。|這會移除選取的具名預設，不會變更任何音訊檔案。
        Transcode.Encoding.Title|Kodierung|Codificación|Encodage|Codifica|Codificação|エンコード|인코딩|编码|編碼
        Transcode.Format.Label|Format|Formato|Format|Formato|Formato|形式|형식|格式|格式
        Transcode.Encoder.Label|Encoder|Codificador|Encodeur|Codificatore|Codificador|エンコーダー|인코더|编码器|編碼器
        Transcode.Encoder.Auto|Automatisch|Automático|Auto|Automatico|Automático|自動|자동|自动|自動
        Transcode.Encoder.OptimFrogOfr|OptimFROG ofr|OptimFROG ofr|OptimFROG ofr|OptimFROG ofr|OptimFROG ofr|OptimFROG ofr|OptimFROG ofr|OptimFROG ofr|OptimFROG ofr
        Transcode.Encoder.OptimFrogOfs|OptimFROG ofs|OptimFROG ofs|OptimFROG ofs|OptimFROG ofs|OptimFROG ofs|OptimFROG ofs|OptimFROG ofs|OptimFROG ofs|OptimFROG ofs
        Transcode.Encoder.OptimFrogOff|OptimFROG off|OptimFROG off|OptimFROG off|OptimFROG off|OptimFROG off|OptimFROG off|OptimFROG off|OptimFROG off|OptimFROG off
        Transcode.RateMode.Label|Ratenmodus|Modo de tasa|Mode de débit|Modalità bitrate|Modo de taxa|レートモード|비트레이트 모드|码率模式|位元率模式
        Transcode.Bitrate.Label|Bitrate (kbps)|Tasa de bits (kbps)|Débit (kbps)|Bitrate (kbps)|Taxa de bits (kbps)|ビットレート (kbps)|비트레이트 (kbps)|码率 (kbps)|位元率 (kbps)
        Transcode.Quality.Label|Qualität|Calidad|Qualité|Qualità|Qualidade|品質|품질|质量|品質
        Transcode.SampleRate.Label|Abtastrate|Frecuencia de muestreo|Fréquence d’échantillonnage|Frequenza di campionamento|Taxa de amostragem|サンプルレート|샘플 레이트|采样率|取樣率
        Transcode.BitDepth.Label|Bittiefe|Profundidad de bits|Profondeur de bits|Profondità in bit|Profundidade de bits|ビット深度|비트 심도|位深度|位元深度
        Transcode.Metadata.Preserve|Metadaten beibehalten|Conservar metadatos|Conserver les métadonnées|Conserva metadati|Preservar metadados|メタデータを保持|메타데이터 유지|保留元数据|保留中繼資料
        Transcode.Artwork.Preserve|Cover beibehalten|Conservar ilustraciones|Conserver les illustrations|Conserva copertine|Preservar arte|アートワークを保持|아트워크 유지|保留封面|保留封面圖片
        Transcode.Correction.Create|Korrekturdatei erstellen, wenn unterstützt|Crear archivo de corrección si es compatible|Créer un fichier de correction si disponible|Crea file di correzione se supportato|Criar arquivo de correção quando compatível|対応時に補正ファイルを作成|지원되는 경우 보정 파일 만들기|支持时创建校正文件|支援時建立修正檔
        Transcode.Destination.Title|Ziel und Benennung|Destino y nombres|Destination et nommage|Destinazione e nomi|Destino e nomenclatura|出力先と名前|대상 및 이름 지정|目标和命名|目的地與命名
        Transcode.Destination.Alongside|Neben den Quelldateien|Junto a los archivos de origen|À côté des fichiers sources|Accanto ai file di origine|Ao lado dos arquivos de origem|ソースファイルの隣|원본 파일 옆|源文件旁边|來源檔案旁
        Transcode.Destination.ReplaceOriginal|Originale ersetzen|Reemplazar originales|Remplacer les originaux|Sostituisci originali|Substituir originais|元ファイルを置換|원본 교체|替换原文件|取代原始檔
        Transcode.Destination.ChosenFolder|Ausgewählter Ordner|Carpeta elegida|Dossier choisi|Cartella scelta|Pasta escolhida|選択したフォルダー|선택한 폴더|所选文件夹|選取的資料夾
        Transcode.Destination.Folder|Ausgabeordner|Carpeta de salida|Dossier de sortie|Cartella di output|Pasta de saída|出力フォルダー|출력 폴더|输出文件夹|輸出資料夾
        Transcode.Destination.PickerTitle|Ausgabeordner für Transkodierung auswählen|Elegir carpeta de salida de transcodificación|Choisir le dossier de sortie du transcodage|Scegli cartella di output della transcodifica|Escolher pasta de saída da transcodificação|変換出力フォルダーを選択|트랜스코딩 출력 폴더 선택|选择转码输出文件夹|選擇轉碼輸出資料夾
        Transcode.Destination.PreserveLayout|Relative Quellstruktur beibehalten|Conservar estructura relativa de origen|Conserver l’arborescence source relative|Conserva struttura relativa di origine|Preservar estrutura relativa de origem|相対ソース構造を保持|상대 원본 구조 유지|保留相对源目录结构|保留相對來源配置
        Transcode.Destination.Flatten|In einen Ordner zusammenfassen|Aplanar en una carpeta|Aplatir dans un dossier|Raggruppa in una cartella|Achatar em uma pasta|1つのフォルダーにまとめる|한 폴더로 평면화|平铺到一个文件夹|攤平到單一資料夾
        Transcode.Naming.Label|Dateibenennung|Nombres de archivo|Nommage des fichiers|Nomi dei file|Nomenclatura de arquivos|ファイル名|파일 이름 지정|文件命名|檔案命名
        Transcode.Naming.Tokens|Token: {Name}, {Extension}, {Codec}, {Encoder}, {SampleRate}, {BitsPerSample}|Tokens: {Name}, {Extension}, {Codec}, {Encoder}, {SampleRate}, {BitsPerSample}|Jetons : {Name}, {Extension}, {Codec}, {Encoder}, {SampleRate}, {BitsPerSample}|Token: {Name}, {Extension}, {Codec}, {Encoder}, {SampleRate}, {BitsPerSample}|Tokens: {Name}, {Extension}, {Codec}, {Encoder}, {SampleRate}, {BitsPerSample}|トークン: {Name}, {Extension}, {Codec}, {Encoder}, {SampleRate}, {BitsPerSample}|토큰: {Name}, {Extension}, {Codec}, {Encoder}, {SampleRate}, {BitsPerSample}|标记：{Name}、{Extension}、{Codec}、{Encoder}、{SampleRate}、{BitsPerSample}|權杖：{Name}、{Extension}、{Codec}、{Encoder}、{SampleRate}、{BitsPerSample}
        Transcode.Collision.Label|Wenn eine Ausgabe vorhanden ist|Cuando existe una salida|Lorsqu’une sortie existe|Quando esiste un output|Quando uma saída existe|出力が存在する場合|출력이 있는 경우|输出已存在时|輸出已存在時
        Transcode.Collision.Stop|Anhalten|Detener|Arrêter|Arresta|Parar|停止|중지|停止|停止
        Transcode.Collision.Suffix|Numerisches Suffix hinzufügen|Agregar sufijo numérico|Ajouter un suffixe numérique|Aggiungi suffisso numerico|Adicionar sufixo numérico|数値サフィックスを追加|숫자 접미사 추가|添加数字后缀|加入數字尾碼
        Transcode.Concurrency.Title|CPU-Auslastung|Uso de CPU|Utilisation du CPU|Utilizzo CPU|Utilização da CPU|CPU 使用率|CPU 사용률|CPU 利用率|CPU 使用率
        Transcode.Concurrency.Automatic|Automatische adaptive Parallelität|Concurrencia adaptativa automática|Parallélisme adaptatif automatique|Concorrenza adattiva automatica|Concorrência adaptativa automática|自動適応並列処理|자동 적응형 동시 실행|自动自适应并发|自動調適並行處理
        Transcode.Tools.Refresh|Werkzeugfunktionen aktualisieren|Actualizar capacidades de herramientas|Actualiser les capacités des outils|Aggiorna funzionalità strumenti|Atualizar recursos das ferramentas|ツール機能を更新|도구 기능 새로 고침|刷新工具功能|重新整理工具功能
        Transcode.Tools.Ready.One|{0:N0} von {1:N0} Werkzeug ist bereit|{0:N0} de {1:N0} herramienta está lista|{0:N0} outil sur {1:N0} est prêt|{0:N0} strumento su {1:N0} è pronto|{0:N0} de {1:N0} ferramenta está pronta|{1:N0} 個中 {0:N0} 個のツールが準備完了|{1:N0}개 도구 중 {0:N0}개 준비됨|{1:N0} 个工具中有 {0:N0} 个就绪|{1:N0} 個工具中有 {0:N0} 個已就緒
        Transcode.Tools.Ready.Other|{0:N0} von {1:N0} Werkzeugen sind bereit|{0:N0} de {1:N0} herramientas están listas|{0:N0} outils sur {1:N0} sont prêts|{0:N0} strumenti su {1:N0} sono pronti|{0:N0} de {1:N0} ferramentas estão prontas|{1:N0} 個中 {0:N0} 個のツールが準備完了|{1:N0}개 도구 중 {0:N0}개 준비됨|{1:N0} 个工具中有 {0:N0} 个就绪|{1:N0} 個工具中有 {0:N0} 個已就緒
        Transcode.Value.Preserve|Quelle beibehalten|Conservar origen|Conserver la source|Conserva origine|Preservar origem|ソースを保持|원본 유지|保留源设置|保留來源
        Transcode.Value.Unavailable|Nicht verfügbar|No disponible|Indisponible|Non disponibile|Indisponível|利用不可|사용할 수 없음|不可用|無法使用
        Transcode.Value.Bits.One|{0:N0} Bit|{0:N0} bit|{0:N0} bit|{0:N0} bit|{0:N0} bit|{0:N0} ビット|{0:N0}비트|{0:N0} 位|{0:N0} 位元
        Transcode.Value.Bits.Other|{0:N0} Bit|{0:N0} bits|{0:N0} bits|{0:N0} bit|{0:N0} bits|{0:N0} ビット|{0:N0}비트|{0:N0} 位|{0:N0} 位元
        Transcode.RateMode.Lossless|Verlustfrei|Sin pérdida|Sans perte|Senza perdita|Sem perdas|ロスレス|무손실|无损|無損
        Transcode.RateMode.ConstantBitrate|Konstante Bitrate (CBR)|Tasa de bits constante (CBR)|Débit constant (CBR)|Bitrate costante (CBR)|Taxa de bits constante (CBR)|固定ビットレート (CBR)|고정 비트레이트 (CBR)|恒定码率 (CBR)|固定位元率 (CBR)
        Transcode.RateMode.AverageBitrate|Durchschnittliche Bitrate (ABR)|Tasa de bits media (ABR)|Débit moyen (ABR)|Bitrate medio (ABR)|Taxa de bits média (ABR)|平均ビットレート (ABR)|평균 비트레이트 (ABR)|平均码率 (ABR)|平均位元率 (ABR)
        Transcode.RateMode.VariableQuality|Variable Qualität (VBR)|Calidad variable (VBR)|Qualité variable (VBR)|Qualità variabile (VBR)|Qualidade variável (VBR)|可変品質 (VBR)|가변 품질 (VBR)|可变质量 (VBR)|可變品質 (VBR)
        Transcode.RateMode.ConstrainedVariableBitrate|Begrenztes VBR|VBR restringido|VBR contraint|VBR vincolato|VBR restrito|制約付き VBR|제한형 VBR|受限 VBR|受限 VBR
        Transcode.RateMode.HybridBitrate|Hybrid-Bitrate|Tasa de bits híbrida|Débit hybride|Bitrate ibrido|Taxa de bits híbrida|ハイブリッドビットレート|하이브리드 비트레이트|混合码率|混合位元率
        Transcode.RateMode.HybridQuality|Hybrid-Qualität|Calidad híbrida|Qualité hybride|Qualità ibrida|Qualidade híbrida|ハイブリッド品質|하이브리드 품질|混合质量|混合品質
        Transcode.Format.AlacM4a|ALAC in M4A|ALAC en M4A|ALAC dans M4A|ALAC in M4A|ALAC em M4A|M4A の ALAC|M4A의 ALAC|M4A 中的 ALAC|M4A 中的 ALAC
        Transcode.Format.AacM4a|AAC in M4A|AAC en M4A|AAC dans M4A|AAC in M4A|AAC em M4A|M4A の AAC|M4A의 AAC|M4A 中的 AAC|M4A 中的 AAC
        Transcode.Format.AacAdts|AAC in ADTS|AAC en ADTS|AAC dans ADTS|AAC in ADTS|AAC em ADTS|ADTS の AAC|ADTS의 AAC|ADTS 中的 AAC|ADTS 中的 AAC
        Transcode.Format.OpusOgg|Opus in Ogg|Opus en Ogg|Opus dans Ogg|Opus in Ogg|Opus em Ogg|Ogg の Opus|Ogg의 Opus|Ogg 中的 Opus|Ogg 中的 Opus
        Transcode.Format.VorbisOgg|Vorbis in Ogg|Vorbis en Ogg|Vorbis dans Ogg|Vorbis in Ogg|Vorbis em Ogg|Ogg の Vorbis|Ogg의 Vorbis|Ogg 中的 Vorbis|Ogg 中的 Vorbis
        Transcode.Format.PcmWave|PCM in WAV|PCM en WAV|PCM dans WAV|PCM in WAV|PCM em WAV|WAV の PCM|WAV의 PCM|WAV 中的 PCM|WAV 中的 PCM
        Transcode.Format.PcmRf64|PCM in RF64|PCM en RF64|PCM dans RF64|PCM in RF64|PCM em RF64|RF64 の PCM|RF64의 PCM|RF64 中的 PCM|RF64 中的 PCM
        Transcode.Format.PcmAiff|PCM in AIFF|PCM en AIFF|PCM dans AIFF|PCM in AIFF|PCM em AIFF|AIFF の PCM|AIFF의 PCM|AIFF 中的 PCM|AIFF 中的 PCM
        Transcode.Format.TrueAudio|True Audio (TTA)|True Audio (TTA)|True Audio (TTA)|True Audio (TTA)|True Audio (TTA)|True Audio (TTA)|True Audio (TTA)|True Audio (TTA)|True Audio (TTA)
        Transcode.Format.OptimFrogDualStream|OptimFROG DualStream (OFS)|OptimFROG DualStream (OFS)|OptimFROG DualStream (OFS)|OptimFROG DualStream (OFS)|OptimFROG DualStream (OFS)|OptimFROG DualStream (OFS)|OptimFROG DualStream (OFS)|OptimFROG DualStream (OFS)|OptimFROG DualStream (OFS)
        Transcode.Format.OptimFrogFloat|OptimFROG Float (OFF)|OptimFROG Float (OFF)|OptimFROG Float (OFF)|OptimFROG Float (OFF)|OptimFROG Float (OFF)|OptimFROG Float (OFF)|OptimFROG Float (OFF)|OptimFROG Float (OFF)|OptimFROG Float (OFF)
        Transcode.Pending.Field|Transkodierung|Transcodificación|Transcodage|Transcodifica|Transcodificação|変換|트랜스코딩|转码|轉碼
        Transcode.Dialog.ReplacePending.Title|Ausstehende Transkodierung ersetzen?|¿Reemplazar la transcodificación pendiente?|Remplacer le transcodage en attente ?|Sostituire la transcodifica in sospeso?|Substituir transcodificação pendente?|保留中の変換を置換しますか？|보류 중인 트랜스코딩을 교체할까요?|替换待处理转码？|取代待處理轉碼？
        Transcode.Dialog.ReplacePending.Message|Mindestens eine ausgewählte Quelle hat bereits eine ausstehende Transkodierung. Die vorherige Transkodierungsabsicht ersetzen?|Al menos un origen seleccionado ya tiene una transcodificación pendiente. ¿Reemplazar su intención anterior?|Au moins une source sélectionnée possède déjà un transcodage en attente. Remplacer son intention précédente ?|Almeno un'origine selezionata ha già una transcodifica in sospeso. Sostituire l'intento precedente?|Pelo menos uma origem selecionada já tem uma transcodificação pendente. Substituir a intenção anterior?|選択したソースの少なくとも1つに保留中の変換があります。以前の変換内容を置換しますか？|선택한 원본 중 하나 이상에 보류 중인 트랜스코딩이 있습니다. 이전 트랜스코딩 설정을 교체할까요?|至少一个所选源已有待处理转码。是否替换其先前的转码意图？|至少一個選取的來源已有待處理轉碼。是否取代先前的轉碼設定？
        Transcode.Dialog.Partial.Title|Einige Dateien konnten nicht vorbereitet werden|No se pudieron preparar algunos archivos|Certains fichiers n’ont pas pu être préparés|Impossibile preparare alcuni file|Alguns arquivos não puderam ser preparados|一部のファイルを準備できませんでした|일부 파일을 준비하지 못했습니다|部分文件无法准备|部分檔案無法準備
        Transcode.Dialog.Partial.Message.One|{0:N0} Datei konnte nicht bereitgestellt werden. {1:N0} bereite Dateien können weiterhin gemeinsam angewendet werden.|No se pudo preparar {0:N0} archivo. Aún se pueden aplicar juntos {1:N0} archivos listos.|{0:N0} fichier n’a pas pu être préparé. {1:N0} fichiers prêts peuvent encore être appliqués ensemble.|Impossibile preparare {0:N0} file. È ancora possibile applicare insieme {1:N0} file pronti.|Falha ao preparar {0:N0} arquivo. {1:N0} arquivos prontos ainda podem ser aplicados juntos.|{0:N0} ファイルのステージングに失敗しました。準備済みの {1:N0} ファイルをまとめて適用できます。|{0:N0}개 파일 스테이징에 실패했습니다. 준비된 {1:N0}개 파일을 함께 적용할 수 있습니다.|{0:N0} 个文件暂存失败，仍可一起应用 {1:N0} 个就绪文件。|{0:N0} 個檔案暫存失敗，仍可一起套用 {1:N0} 個已就緒檔案。
        Transcode.Dialog.Partial.Message.Other|{0:N0} Dateien konnten nicht bereitgestellt werden. {1:N0} bereite Dateien können weiterhin gemeinsam angewendet werden.|No se pudieron preparar {0:N0} archivos. Aún se pueden aplicar juntos {1:N0} archivos listos.|{0:N0} fichiers n’ont pas pu être préparés. {1:N0} fichiers prêts peuvent encore être appliqués ensemble.|Impossibile preparare {0:N0} file. È ancora possibile applicare insieme {1:N0} file pronti.|Falha ao preparar {0:N0} arquivos. {1:N0} arquivos prontos ainda podem ser aplicados juntos.|{0:N0} ファイルのステージングに失敗しました。準備済みの {1:N0} ファイルをまとめて適用できます。|{0:N0}개 파일 스테이징에 실패했습니다. 준비된 {1:N0}개 파일을 함께 적용할 수 있습니다.|{0:N0} 个文件暂存失败，仍可一起应用 {1:N0} 个就绪文件。|{0:N0} 個檔案暫存失敗，仍可一起套用 {1:N0} 個已就緒檔案。
        Transcode.Error.NoReadyFiles|Es ist keine Transkodierungsausgabe zur Anwendung bereit.|No hay ninguna salida transcodificada lista para aplicar.|Aucune sortie transcodée n’est prête à être appliquée.|Nessun output transcodificato è pronto per l'applicazione.|Nenhuma saída transcodificada está pronta para aplicar.|適用可能な変換出力がありません。|적용할 준비가 된 트랜스코딩 출력이 없습니다.|没有可应用的就绪转码输出。|沒有可套用的已就緒轉碼輸出。
        Transcode.Status.Ready|Kodierungseinstellungen auswählen und anschließend die geprüfte Änderung in der Vorschau anzeigen.|Elige la configuración de codificación y luego previsualiza el cambio revisado.|Choisissez les paramètres d’encodage, puis prévisualisez la modification examinée.|Scegli le impostazioni di codifica, quindi visualizza in anteprima la modifica revisionata.|Escolha as configurações de codificação e visualize a alteração revisada.|エンコード設定を選択し、確認対象の変更をプレビューします。|인코딩 설정을 선택한 후 검토할 변경 사항을 미리 봅니다.|选择编码设置，然后预览待审核的更改。|選擇編碼設定，然後預覽待檢閱的變更。
        Transcode.Status.ToolsFailed|Werkzeugfunktionen konnten nicht geladen werden.|No se pudieron cargar las capacidades de las herramientas.|Impossible de charger les capacités des outils.|Impossibile caricare le funzionalità degli strumenti.|Não foi possível carregar os recursos das ferramentas.|ツール機能を読み込めませんでした。|도구 기능을 불러오지 못했습니다.|无法加载工具功能。|無法載入工具功能。
        Transcode.Status.Previewing|Transkodierungseinstellungen werden überprüft…|Validando la configuración de transcodificación…|Validation des paramètres de transcodage…|Convalida delle impostazioni di transcodifica…|Validando configurações de transcodificação…|変換設定を検証しています…|트랜스코딩 설정 검증 중…|正在验证转码设置…|正在驗證轉碼設定…
        Transcode.Status.PreviewBlocked|Beheben Sie die aufgeführten Probleme vor der Vorschau.|Resuelve los problemas indicados antes de la vista previa.|Résolvez les problèmes indiqués avant la prévisualisation.|Risolvi i problemi elencati prima dell'anteprima.|Resolva os problemas listados antes da visualização.|プレビュー前に表示された問題を解決してください。|미리 보기 전에 나열된 문제를 해결하세요.|请先解决列出的问题再预览。|請先解決列出的問題再預覽。
        Transcode.Status.PreviewFailed|Transkodierungsvorschau fehlgeschlagen.|Error en la vista previa de transcodificación.|Échec de la prévisualisation du transcodage.|Anteprima transcodifica non riuscita.|Falha na visualização da transcodificação.|変換プレビューに失敗しました。|트랜스코딩 미리 보기에 실패했습니다.|转码预览失败。|轉碼預覽失敗。
        Transcode.Status.PreviewUnchanged|Die ausstehende Transkodierung wurde nicht geändert.|La transcodificación pendiente no cambió.|Le transcodage en attente n’a pas été modifié.|La transcodifica in sospeso non è stata modificata.|A transcodificação pendente não foi alterada.|保留中の変換は変更されませんでした。|보류 중인 트랜스코딩이 변경되지 않았습니다.|待处理转码未更改。|待處理轉碼未變更。
        Transcode.Status.PreviewReady.One|{0:N0} Transkodierung wurde zu „Änderungen prüfen“ hinzugefügt.|Se añadió {0:N0} transcodificación a Revisar cambios.|{0:N0} transcodage a été ajouté à Examiner les modifications.|{0:N0} transcodifica aggiunta a Rivedi modifiche.|{0:N0} transcodificação adicionada a Revisar alterações.|{0:N0} 件の変換を「変更を確認」に追加しました。|{0:N0}개 트랜스코딩이 변경 사항 검토에 추가되었습니다.|已将 {0:N0} 个转码添加到“审核更改”。|已將 {0:N0} 個轉碼加入「檢閱變更」。
        Transcode.Status.PreviewReady.Other|{0:N0} Transkodierungen wurden zu „Änderungen prüfen“ hinzugefügt.|Se añadieron {0:N0} transcodificaciones a Revisar cambios.|{0:N0} transcodages ont été ajoutés à Examiner les modifications.|{0:N0} transcodifiche aggiunte a Rivedi modifiche.|{0:N0} transcodificações adicionadas a Revisar alterações.|{0:N0} 件の変換を「変更を確認」に追加しました。|{0:N0}개 트랜스코딩이 변경 사항 검토에 추가되었습니다.|已将 {0:N0} 个转码添加到“审核更改”。|已將 {0:N0} 個轉碼加入「檢閱變更」。
        Transcode.Status.PendingAdded.One|{0:N0} ausstehende Transkodierung hinzugefügt|Se añadió {0:N0} transcodificación pendiente|{0:N0} transcodage en attente ajouté|Aggiunta {0:N0} transcodifica in sospeso|Adicionada {0:N0} transcodificação pendente|{0:N0} 件の保留中の変換を追加|{0:N0}개 보류 중인 트랜스코딩 추가됨|已添加 {0:N0} 个待处理转码|已加入 {0:N0} 個待處理轉碼
        Transcode.Status.PendingAdded.Other|{0:N0} ausstehende Transkodierungen hinzugefügt|Se añadieron {0:N0} transcodificaciones pendientes|{0:N0} transcodages en attente ajoutés|Aggiunte {0:N0} transcodifiche in sospeso|Adicionadas {0:N0} transcodificações pendentes|{0:N0} 件の保留中の変換を追加|{0:N0}개 보류 중인 트랜스코딩 추가됨|已添加 {0:N0} 个待处理转码|已加入 {0:N0} 個待處理轉碼
        Transcode.Status.PresetSaved|Voreinstellung gespeichert.|Preajuste guardado.|Préréglage enregistré.|Preimpostazione salvata.|Predefinição salva.|プリセットを保存しました。|프리셋이 저장되었습니다.|预设已保存。|預設已儲存。
        Transcode.Status.PresetUpdated|Voreinstellung aktualisiert.|Preajuste actualizado.|Préréglage mis à jour.|Preimpostazione aggiornata.|Predefinição atualizada.|プリセットを更新しました。|프리셋이 업데이트되었습니다.|预设已更新。|預設已更新。
        Transcode.Status.PresetDeleted|Voreinstellung gelöscht.|Preajuste eliminado.|Préréglage supprimé.|Preimpostazione eliminata.|Predefinição excluída.|プリセットを削除しました。|프리셋이 삭제되었습니다.|预设已删除。|預設已刪除。
        Transcode.Issue.FormatUnavailable|Das ausgewählte Ausgabeformat ist nicht verfügbar.|El formato de salida seleccionado no está disponible.|Le format de sortie sélectionné n’est pas disponible.|Il formato di output selezionato non è disponibile.|O formato de saída selecionado não está disponível.|選択した出力形式は利用できません。|선택한 출력 형식을 사용할 수 없습니다.|所选输出格式不可用。|選取的輸出格式無法使用。
        Transcode.Issue.EncoderUnavailable|Der ausgewählte Encoder ist nicht verfügbar.|El codificador seleccionado no está disponible.|L’encodeur sélectionné n’est pas disponible.|Il codificatore selezionato non è disponibile.|O codificador selecionado não está disponível.|選択したエンコーダーは利用できません。|선택한 인코더를 사용할 수 없습니다.|所选编码器不可用。|選取的編碼器無法使用。
        Transcode.Issue.SourceUnavailable|Eine Quelldatei kann mit den konfigurierten Werkzeugen nicht dekodiert werden.|Un origen no se puede decodificar con las herramientas configuradas.|Une source ne peut pas être décodée avec les outils configurés.|Una sorgente non può essere decodificata con gli strumenti configurati.|Uma origem não pode ser decodificada com as ferramentas configuradas.|構成済みのツールではソースをデコードできません。|구성된 도구로 원본을 디코딩할 수 없습니다.|配置的工具无法解码某个源文件。|已設定的工具無法解碼某個來源檔案。
        Transcode.Issue.SettingsInvalid|Die Kodierungseinstellungen werden vom ausgewählten Encoder nicht unterstützt.|La configuración de codificación no es compatible con el codificador seleccionado.|Les paramètres d’encodage ne sont pas pris en charge par l’encodeur sélectionné.|Le impostazioni di codifica non sono supportate dal codificatore selezionato.|As configurações de codificação não são compatíveis com o codificador selecionado.|選択したエンコーダーはこのエンコード設定に対応していません。|선택한 인코더가 이 인코딩 설정을 지원하지 않습니다.|所选编码器不支持这些编码设置。|選取的編碼器不支援這些編碼設定。
        Transcode.Issue.DestinationInvalid|Ein Ausgabepfad ist nicht verfügbar oder steht mit einer anderen Datei in Konflikt.|Una ruta de salida no está disponible o entra en conflicto con otro archivo.|Un chemin de sortie n’est pas disponible ou entre en conflit avec un autre fichier.|Un percorso di output non è disponibile o è in conflitto con un altro file.|Um caminho de saída não está disponível ou entra em conflito com outro arquivo.|出力パスを利用できないか、別のファイルと競合しています。|출력 경로를 사용할 수 없거나 다른 파일과 충돌합니다.|输出路径不可用或与其他文件冲突。|輸出路徑無法使用或與其他檔案衝突。
        Transcode.Issue.CorrectionUnavailable|Der ausgewählte Encoder kann die angeforderte Korrekturdatei nicht erstellen.|El codificador seleccionado no puede crear el archivo de corrección solicitado.|L’encodeur sélectionné ne peut pas créer le fichier de correction demandé.|Il codificatore selezionato non può creare il file di correzione richiesto.|O codificador selecionado não pode criar o arquivo de correção solicitado.|選択したエンコーダーは要求された補正ファイルを作成できません。|선택한 인코더가 요청한 수정 파일을 만들 수 없습니다.|所选编码器无法创建请求的校正文件。|選取的編碼器無法建立要求的修正檔案。
        Transcode.Issue.DsdSettingsRequired|Für die Umwandlung von DSD in PCM sind Abtastrate und Bittiefe erforderlich.|La conversión de DSD a PCM requiere frecuencia de muestreo y profundidad de bits.|La conversion DSD vers PCM nécessite une fréquence d’échantillonnage et une profondeur de bits.|La conversione da DSD a PCM richiede frequenza di campionamento e profondità di bit.|A conversão de DSD para PCM requer taxa de amostragem e profundidade de bits.|DSD から PCM への変換にはサンプルレートとビット深度が必要です。|DSD-PCM 변환에는 샘플 속도와 비트 심도가 필요합니다.|DSD 转 PCM 需要采样率和位深度。|DSD 轉 PCM 需要取樣率和位元深度。
        Transcode.Issue.ReplaceUnsafe|Diese Quelle kann nicht sicher ersetzt werden, da sie nicht genau ein Audioprogramm enthält.|Este origen no se puede reemplazar de forma segura porque no contiene exactamente un programa de audio.|Cette source ne peut pas être remplacée en toute sécurité, car elle ne contient pas exactement un programme audio.|Questa sorgente non può essere sostituita in sicurezza perché non contiene esattamente un programma audio.|Esta origem não pode ser substituída com segurança porque não contém exatamente um programa de áudio.|音声プログラムが1つだけではないため、このソースを安全に置換できません。|오디오 프로그램이 정확히 하나가 아니므로 이 원본을 안전하게 교체할 수 없습니다.|此源文件并非恰好包含一个音频节目，无法安全替换。|此來源檔案並非恰好包含一個音訊節目，無法安全取代。
        Transcode.Issue.SeparatePrimaryAudio|Die separate Ausgabe enthält nur den primären Audiostream; zusätzliche Streams werden ausgelassen.|La salida separada contendrá solo la secuencia de audio principal; se omitirán las secuencias adicionales.|La sortie séparée contiendra uniquement le flux audio principal ; les flux supplémentaires seront omis.|L'output separato conterrà solo il flusso audio principale; i flussi aggiuntivi verranno omessi.|A saída separada conterá apenas o fluxo de áudio principal; fluxos adicionais serão omitidos.|個別の出力にはプライマリ音声ストリームのみが含まれ、追加のストリームは省略されます。|별도 출력에는 기본 오디오 스트림만 포함되며 추가 스트림은 생략됩니다.|单独输出将仅包含主音频流；其他流将被省略。|個別輸出將僅包含主要音訊串流；其他串流將被省略。
        Transcode.Issue.OutputSessionOnly|Die Ausgabe liegt außerhalb der konfigurierten Indexstammordner und bleibt nur in dieser Sitzung verfügbar.|La salida está fuera de las raíces de índice configuradas y solo permanecerá disponible en esta sesión.|La sortie se trouve hors des racines d’index configurées et restera disponible uniquement dans cette session.|L'output è esterno alle radici dell'indice configurate e rimarrà disponibile solo in questa sessione.|A saída está fora das raízes de índice configuradas e permanecerá disponível apenas nesta sessão.|出力は構成済みのインデックスルート外にあるため、このセッションでのみ利用できます。|출력이 구성된 인덱스 루트 외부에 있으므로 이 세션에서만 사용할 수 있습니다.|输出位于配置的索引根目录之外，因此仅在此会话中可用。|輸出位於已設定的索引根目錄之外，因此僅能在此工作階段使用。
        Transcode.Issue.CatalogRefreshFailed|Die Dateien wurden geändert, aber die betroffenen Bibliothekseinträge konnten nicht aktualisiert werden.|Los archivos se modificaron, pero no se pudieron actualizar las entradas de biblioteca afectadas.|Les fichiers ont été modifiés, mais les entrées de bibliothèque concernées n’ont pas pu être actualisées.|I file sono stati modificati, ma non è stato possibile aggiornare le voci della libreria interessate.|Os arquivos foram alterados, mas não foi possível atualizar as entradas da biblioteca afetadas.|ファイルは変更されましたが、影響を受けるライブラリエントリを更新できませんでした。|파일은 변경되었지만 영향을 받은 라이브러리 항목을 새로 고칠 수 없습니다.|文件已更改，但无法刷新受影响的媒体库条目。|檔案已變更，但無法重新整理受影響的媒體庫項目。
        Transcode.Progress.Planning|Transkodierung wird geplant|Planificando transcodificación|Planification du transcodage|Pianificazione transcodifica|Planejando transcodificação|変換を計画しています|트랜스코딩 계획 중|正在规划转码|正在規劃轉碼
        Transcode.Progress.Preparing|Transkodierung wird vorbereitet|Preparando transcodificación|Préparation du transcodage|Preparazione transcodifica|Preparando transcodificação|変換を準備しています|트랜스코딩 준비 중|正在准备转码|正在準備轉碼
        Transcode.Progress.EncodingUnknown|{0:N0} aktiv · {1:hh\:mm\:ss}|{0:N0} activos · {1:hh\:mm\:ss}|{0:N0} actifs · {1:hh\:mm\:ss}|{0:N0} attivi · {1:hh\:mm\:ss}|{0:N0} ativos · {1:hh\:mm\:ss}|{0:N0} 件を処理中 · {1:hh\:mm\:ss}|{0:N0}개 진행 중 · {1:hh\:mm\:ss}|{0:N0} 个正在进行 · {1:hh\:mm\:ss}|{0:N0} 個正在進行 · {1:hh\:mm\:ss}
        Transcode.Progress.Encoding|{0:N0} aktiv · {1:hh\:mm\:ss} · {2:0}% kodiert|{0:N0} activos · {1:hh\:mm\:ss} · {2:0}% codificado|{0:N0} actifs · {1:hh\:mm\:ss} · {2:0}% encodé|{0:N0} attivi · {1:hh\:mm\:ss} · {2:0}% codificato|{0:N0} ativos · {1:hh\:mm\:ss} · {2:0}% codificado|{0:N0} 件を処理中 · {1:hh\:mm\:ss} · {2:0}% エンコード済み|{0:N0}개 진행 중 · {1:hh\:mm\:ss} · {2:0}% 인코딩됨|{0:N0} 个正在进行 · {1:hh\:mm\:ss} · 已编码 {2:0}%|{0:N0} 個正在進行 · {1:hh\:mm\:ss} · 已編碼 {2:0}%
        Transcode.Progress.Reviewed|{0:N0} Transkodierungsdateien geprüft|Se revisaron {0:N0} archivos de transcodificación|{0:N0} fichiers de transcodage examinés|Esaminati {0:N0} file di transcodifica|{0:N0} arquivos de transcodificação revisados|{0:N0} 件の変換ファイルを確認しました|{0:N0}개 트랜스코딩 파일 검토됨|已审查 {0:N0} 个转码文件|已檢閱 {0:N0} 個轉碼檔案
        Transcode.Issue.StageFailed|Eine Transkodierungsausgabe konnte nicht vorbereitet werden.|No se pudo preparar una salida transcodificada.|Une sortie transcodée n’a pas pu être préparée.|Non è stato possibile preparare un output transcodificato.|Não foi possível preparar uma saída transcodificada.|変換出力を準備できませんでした。|트랜스코딩 출력을 준비하지 못했습니다.|无法准备转码输出。|無法準備轉碼輸出。
        Transcode.Issue.MetadataNotRepresentable|Einige Metadaten können im Ausgabeformat nicht dargestellt werden.|Algunos metadatos no se pueden representar en el formato de salida.|Certaines métadonnées ne peuvent pas être représentées dans le format de sortie.|Alcuni metadati non possono essere rappresentati nel formato di output.|Alguns metadados não podem ser representados no formato de saída.|一部のメタデータは出力形式で表現できません。|일부 메타데이터를 출력 형식으로 표현할 수 없습니다.|部分元数据无法由输出格式表示。|部分中繼資料無法由輸出格式表示。
        Transcode.Issue.ArtworkNotRepresentable|Einige Coverbilder können im Ausgabeformat nicht dargestellt werden.|Algunas ilustraciones no se pueden representar en el formato de salida.|Certaines illustrations ne peuvent pas être représentées dans le format de sortie.|Alcune copertine non possono essere rappresentate nel formato di output.|Algumas artes não podem ser representadas no formato de saída.|一部のアートワークは出力形式で表現できません。|일부 아트워크를 출력 형식으로 표현할 수 없습니다.|部分封面图无法由输出格式表示。|部分封面圖片無法由輸出格式表示。
        Operations.Choice.OperationJournalKind.ReviewedChange|Geprüfte Änderung|Cambio revisado|Modification examinée|Modifica revisionata|Alteração revisada|確認済みの変更|검토된 변경 사항|已审核的更改|已檢閱的變更
        Common.Delete|Löschen|Eliminar|Supprimer|Elimina|Excluir|削除|삭제|删除|刪除
        ...
        About.Package.SkiaSharp.Name|SkiaSharp|SkiaSharp|SkiaSharp|SkiaSharp|SkiaSharp|SkiaSharp|SkiaSharp|SkiaSharp|SkiaSharp
        About.Package.SkiaSharp.Identity|Paket: SkiaSharp|Paquete: SkiaSharp|Paquet : SkiaSharp|Pacchetto: SkiaSharp|Pacote: SkiaSharp|パッケージ: SkiaSharp|패키지: SkiaSharp|软件包：SkiaSharp|套件：SkiaSharp
        About.Package.SkiaSharp.Description|Plattformübergreifende 2D-Grafik- und Bildverarbeitungsbibliothek zur Prüfung und Optimierung von Grafiken.|Biblioteca multiplataforma de gráficos 2D y procesamiento de imágenes para inspeccionar y optimizar las ilustraciones.|Bibliothèque multiplateforme de graphisme 2D et de traitement d’images pour inspecter et optimiser les illustrations.|Libreria multipiattaforma di grafica 2D ed elaborazione delle immagini per ispezionare e ottimizzare la grafica.|Biblioteca multiplataforma de gráficos 2D e processamento de imagens para inspecionar e otimizar ilustrações.|アートワークの検査と最適化に使用するクロスプラットフォームの 2D グラフィックスおよび画像処理ライブラリ。|아트워크 검사 및 최적화를 위한 크로스 플랫폼 2D 그래픽 및 이미지 처리 라이브러리입니다.|用于检查和优化插图的跨平台 2D 图形与图像处理库。|用於檢查和最佳化圖稿的跨平台 2D 圖形與影像處理程式庫。
        About.Package.SkiaSharp.LicenseName|MIT License|MIT License|MIT License|MIT License|MIT License|MIT License|MIT License|MIT License|MIT License
        About.Package.SkiaSharp.LicenseAutomation|SkiaSharp-Lizenzvereinbarung|Acuerdo de licencia de SkiaSharp|Contrat de licence SkiaSharp|Accordo di licenza di SkiaSharp|Contrato de licença do SkiaSharp|SkiaSharp のライセンス契約|SkiaSharp 사용권 계약|SkiaSharp 许可协议|SkiaSharp 授權協議
        About.Package.SkiaSharp.CopyAutomation|SkiaSharp-Lizenzvereinbarung kopieren|Copiar el acuerdo de licencia de SkiaSharp|Copier le contrat de licence SkiaSharp|Copia l'accordo di licenza di SkiaSharp|Copiar o contrato de licença do SkiaSharp|SkiaSharp のライセンス契約をコピー|SkiaSharp 사용권 계약 복사|复制 SkiaSharp 许可协议|複製 SkiaSharp 授權協議
        About.Package.SkiaSharp.LicenseTextAutomation|Vollständiger SkiaSharp-Lizenztext|Texto completo de la licencia de SkiaSharp|Texte complet de la licence SkiaSharp|Testo completo della licenza di SkiaSharp|Texto completo da licença do SkiaSharp|SkiaSharp の完全なライセンス本文|전체 SkiaSharp 사용권 텍스트|完整的 SkiaSharp 许可文本|完整的 SkiaSharp 授權文字
        About.Package.SkiaSharp.NoticesName|Drittherstellerhinweise|Avisos de terceros|Avis de tiers|Avvisi di terze parti|Avisos de terceiros|サードパーティに関する通知|타사 고지 사항|第三方声明|第三方聲明
        About.Package.SkiaSharp.NoticesAutomation|SkiaSharp-Drittherstellerhinweise|Avisos de terceros de SkiaSharp|Avis de tiers de SkiaSharp|Avvisi di terze parti di SkiaSharp|Avisos de terceiros do SkiaSharp|SkiaSharp のサードパーティに関する通知|SkiaSharp 타사 고지 사항|SkiaSharp 第三方声明|SkiaSharp 第三方聲明
        About.Package.SkiaSharp.CopyNoticesAutomation|SkiaSharp-Drittherstellerhinweise kopieren|Copiar los avisos de terceros de SkiaSharp|Copier les avis de tiers de SkiaSharp|Copia gli avvisi di terze parti di SkiaSharp|Copiar os avisos de terceiros do SkiaSharp|SkiaSharp のサードパーティに関する通知をコピー|SkiaSharp 타사 고지 사항 복사|复制 SkiaSharp 第三方声明|複製 SkiaSharp 第三方聲明
        About.Package.SkiaSharp.NoticesTextAutomation|Vollständiger Text der SkiaSharp-Drittherstellerhinweise|Texto completo de los avisos de terceros de SkiaSharp|Texte complet des avis de tiers de SkiaSharp|Testo completo degli avvisi di terze parti di SkiaSharp|Texto completo dos avisos de terceiros do SkiaSharp|SkiaSharp のサードパーティに関する通知の全文|전체 SkiaSharp 타사 고지 사항 텍스트|完整的 SkiaSharp 第三方声明文本|完整的 SkiaSharp 第三方聲明文字
