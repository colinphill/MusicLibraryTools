...
Twelve of those resources carry stronger prior editorial-review provenance,
so status precedence leaves 115 manifest entries as `InvariantApproved`:

```text
Pending=0
InvariantApproved=115
GlossaryReviewed=247
EditorialReviewed=3,291
```

`FocusedEditorialReviewEvidence.v1.xml` is now an intentionally empty,
digest-bound rebase seed. The manifest retains 832 non-stale records from the
earlier focused review set, while the 45 resources added or revised in the
final copy pass are bound to the
...
manifest covers 3,653 resources with no pending review. Ordinary manifest
...
The empty seed was reproducibly generated by comparing the same source commit
on both sides. This records that no new Git-diff review is being claimed while
allowing the manifest to retain every unchanged digest-bound approval:

```powershell
dotnet run --project BuildTools\LocalizationCatalogGenerator `
  --configuration Release -- --check `
  --review-baseline-ref 10fff7626cec65663f615f1771c31cfcebb07c06 `
  --reviewed-ref 10fff7626cec65663f615f1771c31cfcebb07c06 `
  --review-batch gui-usability-final-copy-2026-07-26 `
  --reviewer "Codex usability localization review" `
...
manifest's aggregate digest. The checked-in manifest and strict generator
check verify that the current neutral and all nine translated values still
match every approved digest.
