...
public abstract record StudioDialogRequest(string Title);
public sealed record StudioMessageRequest(string DialogTitle, string Message)
    : StudioDialogRequest(DialogTitle);
public sealed record StudioConfirmRequest(string DialogTitle, string Message, string PrimaryText)
    : StudioDialogRequest(DialogTitle);
public sealed record StudioFieldsRequest(FieldsDialogViewModel ViewModel)
    : StudioDialogRequest(ViewModel.Title);

public sealed class StudioDialogService(IMediaFileService media, ITagWriteService writer)
    : IDialogCoordinator, IFieldsEditorService
{
    private readonly SemaphoreSlim _gate = new(1, 1);
    private TaskCompletionSource<bool>? _completion;

    public StudioDialogRequest? Current { get; private set; }
    public event Action? Changed;

    public async Task<bool> ConfirmAsync(string title, string message, string primaryText) =>
        await ShowAsync(new StudioConfirmRequest(title, message, primaryText));

    public async Task ShowMessageAsync(string title, string message) =>
        await ShowAsync(new StudioMessageRequest(title, message));
    ...
    public async Task<bool> ShowAsync(IReadOnlyList<string> paths)
    {
        ...
        try
        {
            return await ShowAsync(new StudioFieldsRequest(viewModel));
        }
        finally
        {
            viewModel.CloseRequested -= Close;
    ...
    private async Task<bool> ShowAsync(StudioDialogRequest request)
    {
...
public sealed class StudioWorkflowDialogService(
    StudioDialogService dialogs,
