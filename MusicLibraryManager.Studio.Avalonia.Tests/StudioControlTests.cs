...
public sealed class StudioControlTests
{
    [AvaloniaFact]
    public void Data_grid_preserves_studio_metrics_and_column_contract()
    {
        var grid = new StudioDataGrid();
        grid.ConfigureColumns([
            new StudioGridColumnDefinition("Title", "Title", "Title", 280, 140),
            new StudioGridColumnDefinition("Hidden", "Hidden", "Hidden", 120, 80, false),
            new StudioGridColumnDefinition("Path", "Path", "Path", 420, 180),
        ]);
    ...
    [AvaloniaFact]
    public void Studio_buttons_center_content_while_navigation_remains_stretched()
    {
        var studio = new Button { Content = "Run" };
        studio.Classes.Add("studio");
        var navigation = new Button { Content = "Library" };
        navigation.Classes.Add("nav");
        var window = new Window { Content = new StackPanel { Children = { studio, navigation } } };
        try
        {
            window.Show();
            Dispatcher.UIThread.RunJobs();
            Assert.Equal(global::Avalonia.Layout.HorizontalAlignment.Center, studio.HorizontalContentAlignment);
            Assert.Equal(global::Avalonia.Layout.VerticalAlignment.Center, studio.VerticalContentAlignment);
    ...
    public void Split_width_persists_when_the_view_is_recreated()
    {
        ...
        var state = new StudioSplitStateService(settings);
        state.Save("round-trip", 360);

        var split = new StudioSplitView(state)
        {
        ...
        var restored = new StudioSplitView(state)
        {
            PersistenceKey = "round-trip",
            InitialLeftWidth = 300,
            MinLeftWidth = 200,
    ...
    [AvaloniaFact]
    public void Light_and_dark_palette_tokens_match_the_studio_contract()
    {
        Assert.True(Application.Current!.TryGetResource("StudioAccentBrush", ThemeVariant.Dark, out object? darkAccent));
        Assert.True(Application.Current.TryGetResource("StudioCanvasBrush", ThemeVariant.Dark, out object? darkCanvas));
        Assert.True(Application.Current.TryGetResource("StudioAccentBrush", ThemeVariant.Light, out object? lightAccent));
        Assert.True(Application.Current.TryGetResource("StudioCanvasBrush", ThemeVariant.Light, out object? lightCanvas));
    ...
    public void Native_shell_constructs_and_routes_every_destination()
    {
        ...
        foreach ((ShellDestination destination, Type view) in destinations)
        {
            ...
            {
                StudioDataGrid grid = library.FindControl<StudioDataGrid>("LibraryGrid")!;
    ...
    public void Every_destination_completes_a_headless_1440_by_900_render_pass()
    {
        ...
        MainWindow window = services.GetRequiredService<MainWindow>();
        INavigationService navigation = services.GetRequiredService<INavigationService>();
        string? captureDirectory = Environment.GetEnvironmentVariable("STUDIO_CAPTURE_DIR");
        ...
        try
        {
            foreach (ShellDestination destination in Enum.GetValues<ShellDestination>())
            {
                ...
                if (destination == ShellDestination.Library)
                {
                    ...
                    Dispatcher.UIThread.RunJobs();
                    AvaloniaHeadlessPlatform.ForceRenderTimerTick(2);
                    StudioSplitView split = library.FindControl<StudioSplitView>("WorkspaceSplit")!;
