...
| LOC-04 | Add complete `de-DE`, `es-ES`, and `fr-FR` beta catalogs | Complete | 2026-07-24 | `Strings.de-DE.resx`; `Strings.es-ES.resx`; `Strings.fr-FR.resx` | Each satellite has exact parity with all 3,359 neutral resources, with no blanks, fallback markers, or placeholder/protected-token mismatches. |
| LOC-05 | Add complete `it-IT`, `pt-BR`, and `ja-JP` beta catalogs | Complete | 2026-07-24 | `Strings.it-IT.resx`; `Strings.pt-BR.resx`; `Strings.ja-JP.resx` | Each satellite has exact 3,359-key parity and passes deterministic value, placeholder, token, and plural validation. |
| LOC-06 | Add complete `ko-KR`, `zh-CN`, and `zh-TW` beta catalogs | Complete | 2026-07-24 | `Strings.ko-KR.resx`; `Strings.zh-CN.resx`; `Strings.zh-TW.resx` | Each satellite has exact 3,359-key parity and passes deterministic value, placeholder, token, plural, and CJK-residue validation. |
...
| TST-09 | Complete Release build/test and Windows/Linux/macOS publish/package verification | Complete | 2026-07-24 | Portable solution; package outputs; signing catalog | Final Release build: 0 warnings/errors. Final tests: 1,822/1,822 (955 Core, 205 presentation, 78 UI, 506 utilities, 78 DumpITL). Win-x64 and linux-x64 archives were generated with checksums; all four RID publishes contain nine satellites and four embedded syncer daemons. A 17-entry signing catalog was generated. Real Authenticode verification needs release credentials; DMG verification runs on macOS CI. |
...
- Completed scope: adaptive reverse-delta metadata recovery, exact stale-safe restartable undo, legacy compatibility, nine complete beta locales, live localized grid/editor behavior, deterministic catalog tooling, resource-aware packaging/signing, and the requested validation coverage.
- Remaining issues: no code issue is open. Credentialed signing and macOS DMG creation are external release gates.
- Test results: Release build succeeded with 0 warnings/errors; 1,822/1,822 solution tests passed; localization generator validated 9 x 3,359 resources; responsive matrix passed 2/2; focused compact/codec passed 50/50 and broader recovery passed 76/76.
