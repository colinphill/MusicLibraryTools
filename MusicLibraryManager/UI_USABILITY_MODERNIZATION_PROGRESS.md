...
- Destructive-scope validation binds each enabled action to its visible identity or reviewed count: Workbench file operations, all four Health repair families, Devices, Ingest, Organize, Operations restore/purge, and Settings root/profile removals are covered.
- Localization generator validation: nine satellite catalogs with exact parity at 3,544 resources each.
- Editorial localization audit: 1,406 current keys are `EditorialReviewed`, 115 exact neutral-value invariants are approved, 71 built-in or exact-resource translations are `GlossaryReviewed`, and 1,952 contextual keys remain Pending. The versioned manifest binds ordered key, catalog digest, status, route, review batch, reviewer, date, and disposition.
...
- Localization provenance validation: the real generator validated all nine catalogs at 3,544 resources each; the Ingest checkpoint passed 76/76 focused localization/manifest/Ingest tests plus 2/2 real-generator command-line gates. Strict review correctly exits with `Strict editorial review failed: 1,952 resources remain Pending.`
...
| DEL-04 | Deliver Workbench section and drawer corrections | Complete | 2026-07-25 | Workbench host, sections, drawers | Section and drawer redesign is implemented. The complete 1,080-case Workbench presentation matrix, exact remaining-task-width constraints, all width/height thresholds, strict action reachability, localized narrow pending actions, and independent forward/reverse focus cycles for Inspector, Review Changes, Columns, and Transcode pass. File-operation scope/count occupies an always-visible row outside its form scroll. |
...
| WB-06 | Unify Inspector, Review Changes, Columns, and Transcode drawer host | Complete | 2026-07-25 | Workbench drawer host/views | Shared host, bounds, scrim, Escape, sticky footer, focus restoration, and forward/reverse focus cycling are implemented. Pending actions wrap within narrow German/18 px drawers without clipping. Independent interaction tests cover Inspector, Review Changes, Columns, and Transcode in the green 157-test UI suite. |
...
| LOC-02 | Polish all nine beta satellite catalogs | In Progress | 2026-07-25 | Nine satellite catalogs; `EditorialOverrides.xml`; `EditorialReviewManifest.xml`; review evidence/invariant baseline | Structural parity is green at 9×3,544. The digest-bound manifest currently records 1,406 EditorialReviewed, 115 InvariantApproved, 71 GlossaryReviewed, and 1,952 Pending keys. The exact small/workflow, Operations, Devices, and Ingest batches together cover 630 contexts with complete nine-locale review. They add 151 clearer en-US values and 509 complete correction maps; Operations also adds compound-filename protection for `iTunes Library.itl`. Manifest and evidence identities reject stale catalog values, reordered/injected packets, altered routes, and recomputed-but-inauthentic aggregate digests. Health, Library, and Settings artifacts remain for later checkpoints. Completion requires 3,544/3,544 current approvals and strict review success. |
...
| VAL-03 | Add non-overlap, reachability, monotonic-layout, and primary-action assertions | Complete | 2026-07-25 | UI tests; source validators; `UiViewportReachability` | Every live adaptive boundary has exact −1/0/+1 coverage. Primary/danger actions must be fully visible or proven reachable through a usable vertical scroll owner; probes restore every prior offset. Cross-app matrix assertions cover page overflow, action presence/reachability, live localization, responsive monotonicity, shell overlays, and capture identity. A dedicated gate proves visible identity/count relationships for Workbench file operations, Health repairs, Devices, Ingest, Organize, Operations restore/purge, and Settings root/profile removals. These gates exposed and fixed task-width regressions and localized pending-action clipping. |
...
- Catalog parity and override presence are structural evidence only. Editorial completion is enforced by a separate digest-bound review manifest; the current audit identifies 1,952 contextual keys still requiring focused review.
- Open blockers: none.
...
- Remaining scope: review and manifest the 1,952 outstanding localization contexts; generate and review fresh captioned screenshots/contact sheets; run complete solution and publish/package verification.
- Current test results: the current core suite passes 325/325 and the preceding complete UI suite passes 157/157; the current artwork-preview subset passes 7/7. Workbench remains green at 1,080/1,080 presentations, cross-app at 1,200/1,200, representative states at 1,320/1,320, and localization at 9×3,544 resources.
